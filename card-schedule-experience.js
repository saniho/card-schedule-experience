  ...
  class CardScheduleExperience extends LitElement {
    static get properties() {
      return {
        ...
        _editingScenarioId: { state: true }, // ID du scénario en cours d'édition
      };
    }
    ...
    constructor() {
      ...
      this.conditionTypes = [ { value: 'temperature', label: 'Température' }, { value: 'luminosity', label: 'Luminosité' }, { value: 'presence', label: 'Présence' }, { value: 'humidity', label: 'Humidité' } ];
      this.operators = [ { value: '<', label: '<' }, { value: '<=', label: '≤' }, { value: '=', label: '=' }, { value: '>=', label: '≥' }, { value: '>', label: '>' } ];
      this.actionTypes = [ { value: 'heater', label: 'Chauffage' }, { value: 'light', label: 'Lumière' }, { value: 'notification', label: 'Notification' }, { value: 'switch', label: 'Interrupteur' } ];

      // Données d'exemple avec structure de règles
      this._timeslots = [
      ...
      this._scenarios = [
        { id: '1', name: 'Chauffage confort', color: '#3b82f6', rules: [ { id: 'r1', conditions: [{ type: 'temperature', operator: '<', value: 18 }], actions: [{ type: 'heater', entity: 'climate.salon', state: 'heat', temp: 21 }] } ] },
    ...
    _addCondition(scenarioId, ruleId) { const cond = { type: 'temperature', operator: '=', value: 20 }; this._scenarios = this._scenarios.map(s => s.id === scenarioId ? { ...s, rules: s.rules.map(r => r.id === ruleId ? { ...r, conditions: [...r.conditions, cond] } : r) } : s); this.requestUpdate(); }
    ...
    _addAction(scenarioId, ruleId) { const action = { type: 'switch', entity: '', state: 'on' }; this._scenarios = this._scenarios.map(s => s.id === scenarioId ? { ...s, rules: s.rules.map(r => r.id === ruleId ? { ...r, actions: [...r.actions, action] } : r) } : s); this.requestUpdate(); }
    ...
    // NOUVEAU: Rendu des scénarios
    _renderScenarios() { return html`<div class="scenarios-list">${this._scenarios.map(s => this._renderScenarioItem(s))}</div><mwc-button outlined label="Ajouter un scénario" icon="add" @click=${this._addScenario}></mwc-button>`; }
    ...
    _renderScenarioEditor(scenario) { return html`<div class="scenario-editor">${this._renderScenarioDetails(scenario)}${scenario.rules.map(rule => this._renderRule(scenario, rule))}<mwc-button outlined label="Ajouter une règle" icon="add_circle_outline" @click=${() => this._addRule(scenario.id)}></mwc-button></div>`; }
    ...
    _renderRule(scenario, rule) { return html`<div class="rule-card"><div class="rule-header"><span>Règle</span><ha-icon-button class="delete-btn-panel" label="Supprimer Règle" @click=${() => this._deleteRule(scenario.id, rule.id)}><ha-icon icon="hass:trash-can-outline"></ha-icon></ha-icon-button></div><div class="rule-content"><strong>SI</strong><div class="conditions">${rule.conditions.map((cond, i) => this._renderCondition(scenario, rule, cond, i))}<mwc-button dense outlined label="Ajouter condition" @click=${() => this._addCondition(scenario.id, rule.id)}></mwc-button></div><strong>ALORS</strong><div class="actions">${rule.actions.map((act, i) => this._renderAction(scenario, rule, act, i))}<mwc-button dense outlined label="Ajouter action" @click=${() => this._addAction(scenario.id, rule.id)}></mwc-button></div></div></div>`; }
    _renderCondition(scenario, rule, cond, index) { return html`<div class="condition-row"><ha-select .value=${cond.type} @selected=${e => this._updateCondition(scenario.id, rule.id, index, { type: e.target.value })}>${this.conditionTypes.map(o => html`<mwc-list-item .value=${o.value}>${o.label}</mwc-list-item>`)}</ha-select><ha-select .value=${cond.operator} @selected=${e => this._updateCondition(scenario.id, rule.id, index, { operator: e.target.value })}>${this.operators.map(o => html`<mwc-list-item .value=${o.value}>${o.label}</mwc-list-item>`)}</ha-select><ha-textfield type="number" .value=${cond.value} @change=${e => this._updateCondition(scenario.id, rule.id, index, { value: e.target.value })}></ha-textfield><ha-icon-button class="delete-btn-panel" @click=${() => this._deleteCondition(scenario.id, rule.id, index)}><ha-icon icon="hass:close"></ha-icon></ha-icon-button></div>`; }
    _renderAction(scenario, rule, act, index) { return html`<div class="action-row"><ha-select .value=${act.type} @selected=${e => this._updateAction(scenario.id, rule.id, index, { type: e.target.value })}>${this.actionTypes.map(o => html`<mwc-list-item .value=${o.value}>${o.label}</mwc-list-item>`)}</ha-select><ha-textfield label="Entité" .value=${act.entity} @change=${e => this._updateAction(scenario.id, rule.id, index, { entity: e.target.value })}></ha-textfield><ha-textfield label="État" .value=${act.state} @change=${e => this._updateAction(scenario.id, rule.id, index, { state: e.target.value })}></ha-textfield>${act.type === 'heater' ? html`<ha-textfield label="Temp" type="number" .value=${act.temp} @change=${e => this._updateAction(scenario.id, rule.id, index, { temp: e.target.value })}></ha-textfield>` : ''}<ha-icon-button class="delete-btn-panel" @click=${() => this._deleteAction(scenario.id, rule.id, index)}><ha-icon icon="hass:close"></ha-icon></ha-icon-button></div>`; }
...
console.info(
  `%c CARD-SCHEDULE-EXPERIENCE %c v0.0.7 `,
