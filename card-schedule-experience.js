  ...
  class CardScheduleExperience extends LitElement {
    ...
    _renderCondition(scenario, rule, cond, index) {
      ...
      return html`<div class="condition-row">
        <ha-select .label="Entité" .value=${cond.entity} @selected=${e => this._updateCondition(scenario.id, rule.id, index, { entity: e.target.value })}>
    ...
    _renderAction(scenario, rule, act, index) {
      ...
      return html`<div class="action-row">
        <ha-select .label="Entité" .value=${act.entity} @selected=${e => this._updateAction(scenario.id, rule.id, index, { entity: e.target.value })}>
          ${entities.map(entity => html`<mwc-list-item .value=${entity}>${this.hass.states[entity].attributes.friendly_name || entity}</mwc-list-item>`)}
        </ha-select>
...
console.info(
  `%c CARD-SCHEDULE-EXPERIENCE %c v0.0.19 `,
