function defineCard(LitElement, html, css) {
  class CardScheduleExperience extends LitElement {
    ...
    getAutomationColor(automationId) {
      if (this._automationColors[automationId]) {
        return this._automationColors[automationId];
      }
      // Couleur par défaut
        ...
        <ha-card>
          <div class="header">
            ...
            <div class="info-container">
              ...
              <div class="card-description">Planification des automations</div>
            </div>
          </div>
          <div class="content">${this._renderTimeslots(days)}</div>
        </ha-card>
    ...
    _renderEditPanel() {
      ...
      if (!s) return '';
      const htc = (e, f) => {
        const ps = { ...s, [f]: e.target.value };
        if (this.checkConflict(ps)) {
          alert(`Conflit avec ${this.checkConflict(ps).name}.`);
          e.target.value = s[f];
        } else this.updateTimeslot(s.id, { [f]: e.target.value });
      };
      const hd = () => {
        if (confirm(`Supprimer "${s.name}" ?`)) this.deleteTimeslot(s.id);
      };
      ...
      const selectedAutoColor = s.automationId ? this._automationColors[s.automationId] || this.getAutomationColor(s.automationId) : '#3b82f6';

      return html`<div class="edit-panel">
        <div class="edit-header">
          <h3>Édition de la plage</h3>
          <div class="header-buttons">
            <ha-icon-button class="delete-btn-panel" .label="Supprimer" @click=${hd}>
              <ha-icon icon="hass:trash-can-outline"></ha-icon>
            </ha-icon-button>
            <ha-icon-button .label="Fermer" @click=${()=>this._editingSlot=null}>
              <ha-icon icon="hass:close"></ha-icon>
            </ha-icon-button>
          </div>
        </div>
        <div class="edit-content">
          <ha-textfield label="Début" type="time" .value=${s.startTime} @change=${(e)=>htc(e,'startTime')}></ha-textfield>
          <ha-textfield label="Fin" type="time" .value=${s.endTime} @change=${(e)=>htc(e,'endTime')}></ha-textfield>
          ...
          <select .value=${s.automationId} @change=${(e)=>{
            ...
            ${automations.map(auto=>html`<option .value=${auto.id}>${auto.name}</option>`)}
          </select>
          ${s.automationId ? html`
            <label>Couleur de l'automation</label>
            <div class="color-input-wrapper">
              <input type="color" .value=${selectedAutoColor} @input=${(e)=>{
                this.setAutomationColor(s.automationId, e.target.value);
              }} />
              <span class="color-label">${selectedAutoColor}</span>
            </div>
          ` : ''}
        </div>
      </div>`;
