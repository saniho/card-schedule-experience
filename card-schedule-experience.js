function defineCard(LitElement, html, css) {
  class CardScheduleExperience extends LitElement {
    ...
    _renderCondition(scenario, rule, cond, index) {
      const entityPicker = customElements.get('ha-entity-picker')
        ? html`<ha-entity-picker .hass=${this.hass} .value=${cond.entity} .label="Entité" @value-changed=${e => this._updateCondition(scenario.id, rule.id, index, { entity: e.detail.value })} allow-custom-entity></ha-entity-picker>`
        : html`<ha-textfield label="Entité" .value=${cond.entity} @change=${e => this._updateCondition(scenario.id, rule.id, index, { entity: e.target.value })}></ha-textfield>`;
    ...
    _renderAction(scenario, rule, act, index) {
      const entityPicker = customElements.get('ha-entity-picker')
        ? html`<ha-entity-picker .hass=${this.hass} .value=${act.entity} .label="Entité" @value-changed=${e => this._updateAction(scenario.id, rule.id, index, { entity: e.detail.value })} allow-custom-entity></ha-entity-picker>`
        : html`<ha-textfield label="Entité" .value=${act.entity} @change=${e => this._updateAction(scenario.id, rule.id, index, { entity: e.target.value })}></ha-textfield>`;
...
(async () => {
  // This is a robust way to ensure that all required components are loaded before defining our card
...
console.info(
  `%c CARD-SCHEDULE-EXPERIENCE %c v0.0.18 `,
